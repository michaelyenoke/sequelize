...
.then(() => {
    ...
    // Executing (default): UPDATE `customer` SET `deletedAt`=? WHERE `deletedAt` IS NULL AND `user_id` = ? (note: "?" -> use the binding)
    // 1

    // Deleting a Record from a Paranoid Table
    return Customer.destroy({ where: {user_id:3}})
